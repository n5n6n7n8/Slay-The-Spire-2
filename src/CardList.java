...
public class CardList {
    ...
    public CardList(Player player, Enemy enemy){//List of all cards
        ...
        allCards.add(new Card("Kunai", CardType.STAR, "Deal 3 dmg. Exhaust", 0, e -> enemy.loseHealth(3+player.getStrength()), true));
        ...
        allCards.add(new Card("Flechettes", CardType.ATTACK, "For every attack in your deck, deal 3 dmg", 2, e -> {
            if(player.getEnergy()>=2){
                enemy.loseHealth(player.countCardsByType(CardType.ATTACK)*3+(player.getStrength()));
            }
        }));
...
//        }));
        ...
        allCards.add(new Card("Milk", CardType.FRAGILE, "Deal 4 dmg. Apply 1 fragile.", 1, e -> {
            if(player.getEnergy()>=2){
        ...
        allCards.add(new Card(true));//only curse card lol
